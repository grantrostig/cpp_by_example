...
void test_02() {  using namespace test_02_ns; cout << "Start test 02" << endl;
    ...
    //i_conFactory_uptr.reset( new UDPConnectionFactory );              // TODO??:
...
#else
                                            //use_factory_hello(move(i_conFactory_uptr));                       // BAD: We loose posession TODO??: permanently?
    ...
    unique_ptr<I_ConnectionFactory> i_conFactory_uptr_maked{make_unique<TCPConnectionFactory>()};   // Yes obviously, but still checking it. :)
#ifdef PASS_BY_REF
    use_factory_hello(*i_conFactory_uptr_maked);
#else
    use_factory_hello(move(i_conFactory_uptr_maked));
#endif
    ...
    i_conFactory_uptr.reset(nullptr);
    i_conFactory_uptr_maked.reset(nullptr);
