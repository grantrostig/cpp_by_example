...
public:
    ...
    void start() { 		// Initiate the asynchronous operations associated with the connection.
        read_request();
        check_deadline();
    }
...
private:
    ...
    net::steady_timer 						deadline_	{ 		// The timer for putting a deadline on connection processing.
    ...
    void create_response() { 							// Construct a response message based on the program state.
        if(request_.target() == "/count") {
            ...
            response_.set(http::field::content_type, "text/plain");
            beast::ostream(response_.body()) << "Beast run() Error: 404 File not found\r\n";
        }
    }
...
void http_server(tcp::acceptor& acceptor, tcp::socket& socket) {
  acceptor.async_accept( socket, [&](beast::error_code ec) {
          if(!ec) {
              auto ret = std::make_shared<http_connection>( std::move(socket) );
              ret -> start();
          }
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::cerr << "Beast run() Error:" << e.what() << std::endl;
        return EXIT_FAILURE;
    }
}
