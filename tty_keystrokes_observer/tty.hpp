...
/// An pure virtual function acting as an conceptual "interface", being a collection of functions (tk_arrow...)
...
/// A class representing a "tty physical/logical interface",
/// which connects a terminal or xterminal to a running Linux instance via a device driver.
/// It "is a" GOF "Subject" representing keystroke events, or the end user pressing the keyboard with one keystroke.
/// These events are what the GOF "subjects" are "Observing", those observers are named "TODO??:XXXXX" in this program.
class Tty : public Subject< Keystroke_events_I > {
    ...
    tcflag_t                  _orig_tty_cflags            {};     // TODO??: why not 0 zero? or is nothing zero?
    tcflag_t            config_tty_to_raw();
    void                restore_tty();
    void                tty_read_thread();
public:
    ~Tty();
    void                read_keystrokes_from_keyboard();
    void                stop();
private:                                            // The const bytes that comprise a keystroke on POSIX xterm keyboards.
